...
class LearnTable():
    ...
    def add_experimental_columns_to_table(self, dic):
        ...
        if dic["experiment_name"]=="MST":
            ...
            df = df.assign(MST_q_real_t = [0] * self.df.shape[0])
        self.df = df
        #print(df.head)
    ...
    def add_to_table(self, dic): 
        ...
        for subgroup in dic["_ids"]:
            ...
            for filename in files:
                ...
                row_num = self.get_row_number_of_subject_from_file(filename, dic)
    ...
    def add_results_to_table(self, dic, row_num):
        if dic["experiment"]=="MST":
            self.add_MST_results_to_table(dic, row_num)


    def add_MST_results_to_table(self, dic, row_num):
        df = self.df
        df.loc[row_num,'MST_abs_corr_seq'] = dic["abs_corr_seq"]
    ...
    def get_row_number_of_subject_from_file(self, filename, dic):
        if dic["experiment_name"]=="MST":
            ...
            #vpn = int(filename[:1])
            vpn = 10
        return self.df.loc[self.df['VPN']==vpn].iloc[0,0]
