...
class Group():
    def __init__(self, experiment = 'MST', path_inputfiles="./Data MST", filepattern="Tag1", path_outputfiles = ".\\Data_python", _id = None, sequence_length=10):
        ...
        if not _id:
            self._id = datetime.datetime.today().strftime('%Y%m%d_%H%M%S')
        else:
            self._id = _id
        self.get_data()
