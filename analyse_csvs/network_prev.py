...
#        self.ipi = self.convert_to_array2D(self.ipi_norm)
        ...
        if self.is_estimate_Q:
            self.g_real, self.q_real = self.estimate_chunks(is_random = False)
            self.phi_real = self.estimate_chunk_magnitudes(self.g_real)
...
#        P = self.get_adjacency_matrix(ipi_shuffled)
    ...
    def estimate_chunks(self, is_random=False):
        ...
        if is_random:
            ...
            #A = self.get_adjacency_matrix(self.ipi)
        else:
